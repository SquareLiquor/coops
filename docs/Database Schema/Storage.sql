...
-- 조회: 모든 사용자 가능, coops 버킷만
create policy "Allow all selects on coops bucket"
on storage.objects
for select
using (bucket_id = 'coops');

-- 삽입: 로그인한 사용자만, coops 버킷만
create policy "Allow inserts for authenticated users on coops"
on storage.objects
for insert
with check (bucket_id = 'coops' AND auth.uid() = owner);

-- 업데이트: 로그인한 사용자만 자신의 row, coops 버킷만
create policy "Allow updates for owners on coops"
on storage.objects
for update
using (bucket_id = 'coops' AND auth.uid() = owner);

-- 삭제: 로그인한 사용자만 자신의 row, coops 버킷만
create policy "Allow deletes for owners on coops"
on storage.objects
for delete
using (bucket_id = 'coops' AND auth.uid() = owner);
