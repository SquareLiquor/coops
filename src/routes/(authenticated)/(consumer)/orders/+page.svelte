...
<div class="min-h-screen bg-gradient-to-br from-surface-50/50 to-surface-50/10">
  <!-- Filter Section -->
  <div
    class="border-b border-surface-200 bg-gradient-to-b from-primary-500/5 to-white px-4 pt-2 pb-4 dark:border-surface-700 dark:from-primary-500/5 dark:to-surface-800"
  >
    <div class="container mx-auto">
      <div class="flex items-center space-x-4">
        ...
        <div class="flex-shrink-0">
          <button
            class="flex items-center space-x-1.5 rounded-lg border border-surface-300 bg-white px-2.5 py-1.5 transition-colors hover:border-primary-500 dark:border-surface-600 dark:bg-white"
          >
            <svg class="h-3.5 w-3.5 text-surface-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path
            ...
            <span class="text-xs font-medium text-surface-700">{formatDate(selectedDate)}</span>
          </button>
        </div>
          ...
          {#each statusOptions as status}
            <button
              class={selectedStatus === status
                ? 'rounded-full bg-primary-500 px-3 py-1 text-sm font-medium whitespace-nowrap text-white'
                : 'rounded-full border border-surface-200 bg-white px-3 py-1 text-sm font-medium whitespace-nowrap text-surface-700 transition-colors hover:bg-surface-50 dark:bg-white dark:text-surface-700 dark:hover:bg-surface-50'}
  ...
  <main class="container mx-auto px-4 py-4 pb-20">
    {#if filteredOrders.length === 0}
      ...
      <div class="flex flex-col items-center justify-center py-16 text-center">
        <svg class="mb-4 h-16 w-16 text-surface-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path
            stroke-linecap="round"
            stroke-linejoin="round"
            stroke-width="2"
        ...
        <h3 class="mb-2 text-lg font-semibold text-surface-900 dark:text-surface-100">주문 내역이 없습니다</h3>
    ...
    {:else}
      ...
      <div class="space-y-4">
        {#each filteredOrders as order}
          <div
            class="overflow-hidden rounded-xl border border-surface-200 bg-white shadow-sm dark:border-surface-700 dark:bg-surface-800"
          >
            <!-- Order Header -->
            <div class="border-b border-dashed border-surface-300 p-4 dark:border-surface-600">
              <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3">
                  ...
                  >
                    <div class="text-sm font-semibold text-primary-600 transition-colors hover:text-primary-700">
                      주문번호 #{order.id}
                    </div>
            ...
            <!-- Order Footer -->
            <div class="border-t border-dashed border-surface-300 bg-white p-4 dark:border-surface-600 dark:bg-white">
              <div class="flex items-center justify-between">
